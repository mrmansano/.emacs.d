...
;;; Custom commands
;; Emacs utilities
(ex! "bc[omp]"      '+mrmansano:byte-compile)
...
(ex! "enhtml"       '+mrmansano:encode-html-entities)
(ex! "dehtml"       '+mrmansano:decode-html-entities)
(ex! "ie[dit]"      'evil-multiedit-ex-match)
(ex! "na[rrow]"     '+mrmansano:narrow-buffer)
(ex! "ref[actor]"   'emr-show-refactor-menu)
(ex! "retab"        '+mrmansano:retab)
...
(ex! "k[ill]all"   '+mrmansano:kill-all-buffers)
(ex! "k[ill]m"     '+mrmansano:kill-matching-buffers)
